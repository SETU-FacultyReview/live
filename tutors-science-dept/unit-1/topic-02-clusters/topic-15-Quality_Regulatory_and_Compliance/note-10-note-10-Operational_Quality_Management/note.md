---
icon:
  type: mdi:certificate-outline
  color: 00897B
---
# Operational QM

The aim of this module is to provide the professional adult learner with knowledge relating to quality and operations management, lean manufacturing, six sigma, leadership, strategy and competitive advantage which they can apply to their workplace environment.

<p><a href="./archives/A33461.pdf" target="_blank" rel="noopener noreferrer" style="display: inline-flex; align-items: center; text-decoration: none;"><img src="https://upload.wikimedia.org/wikipedia/commons/6/60/Adobe_Acrobat_Reader_icon_%282020%29.svg" width="20" height="20" style="margin-right: 4px;" alt="Adobe Acrobat Icon"><span>Module Descriptor (PDF)</span></a></p>

## Module Information

| **Field** | **Details** |
|-----------|-------------|
| **Module Code** | A33461 |
| **Module Title** | Operational Quality Management |
| **Short Title** | Operational QM |
| **Credits** | 10 ECTS |
| **Level** | Advanced |
| **School** | Science and Computing |
| **Department** | Science |
| **Module Author** | BWHELAN |
| **Cluster** | [Quality, Regulatory and Compliance](/topic/setu-science-dept/unit-1/topic-02-clusters/topic-15-Quality_Regulatory_and_Compliance) |

---

## Module Aim

The aim of this module is to provide the professional adult learner with knowledge relating to quality and operations management, lean manufacturing, six sigma, leadership, strategy and competitive advantage which they can apply to their workplace environment.

---

## Learning Outcomes

On successful completion of this module, learners will be able to:

1. Appraise the application of Lean Six Sigma and associated systems in the workplace.
2. Critique the impact of strategic management and effective leadership.
3. Evaluate adaptive organisational culture.
4. Assess change management initiatives using case studies.
5. Interpret the application of performance metrics to maximise competitive advantage.

---

## Indicative Content

The module covers the following topics:

- Lean Management Systems, Six Sigma, Operational Excellence
- Strategy and Leadership
- Organisational culture and management
- Change Management, global context for change
- Performance Management

---

## Learning and Teaching Methods

Lectures

Case studies

Group discussion of articles

Seminars

Independent study

### Contact Hours

| **Activity** | **Full Time Hours** | **Part Time Hours** |
|--------------|---------------------|---------------------|
| Lecture | 0 | 24 |
| Independent Learning | 0 | 246 |
| **Total** | **0** | **270** |

---

## Assessment Methods

| **Assessment Type** | **Learning Outcomes** | **Weighting** |
|---------------------|----------------------|---------------|
| **Continuous Assessment** | 1, 2, 3, 4, 5 | **100%** |
| - Assignment | 1, 2, 3 | 50% |
| - Case Studies | 4 | 30% |
| - Presentation | 5 | 20% |

---

## Assessment Criteria

< 40%: Little or no demonstration of an understanding of the fundamentals of the learning outcomes. Lack of competence with associated laboratory skills & practices & an inability to carry out critical thinking.

40%--49%: Will show a limited understanding of the fundamentals of the learning outcomes. May have some difficulty with applying critical thinking and problem-solving. Will display some ability with associated laboratory skills & practices.

50%--59%: As well as the above, the candidate will show a good understanding of the fundamentals of the learning outcomes with demonstration of some critical thinking & problem-solving skills. Will display good competence with associated laboratory skills & practices.

60%--69%: In addition, will demonstrate more detailed knowledge of all learning outcomes, very good critical thinking skills & a high level of competence & efficiency in associated laboratory practicals.

70%--100%: All the above to an excellent level. In addition, will demonstrate a comprehensive knowledge & understanding of all learning outcomes & be able to discuss the area. Will display advanced competence & independence with associated laboratory skills & practices.

---

## Pre-requisites and Co-requisites

- **Pre-requisites:** None
- **Co-requisites:** None

---

## Recommended Reading

### Supplementary Material

- "Harvard Business Review." https://hbr.org
- Jeffs, C. _Strategic Management_. 1st. XX: Sage Publications, 2012.
- Schein, E. _Organisational Culture & Leadership_. 5th. XX: Wiley & Sons, 2017.

---

### Essential Material

- Burnes, B. _Managing Change: a strategic approach to organisational dynamics_. 7th. XX: Pearson, 2017.
- Mann, D. _Creating a Lean Culture: Tools to sustain Lean Conversions_. 3rd. XX: Productivity Press, 2015.

---

## Programme Information

This module is available on the following programmes:

| **Programme Code** | **Programme Title** | **Stage** | **Semester** | **Status** |
|-------------------|---------------------|-----------|--------------|------------|
| WD_SGMPT_B | BSc (Hons) in Good Manufacturing Practice and Technology | 4 | 2 | Mandatory |

---

## Resources Required


---

*Module Code: A33461 | Timetable Code: QUAL-0034 | Change Tracking: Enabled*